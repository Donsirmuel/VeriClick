...
import { RoboticIcon, ArrowRight01Icon, InformationCircleIcon } from '@hugeicons/core-free-icons'
import { formatRelativeTime } from '@/lib/utils'
import type { ActivityEntry } from '@/types'

export function BlockedQueueWidget({ activity }: { activity: ActivityEntry[] }) {
  const blocked = activity.filter((e) => e.isBot).slice(0, 5)
        ...
        {blocked.map((entry) => (
          <div key={entry.id} className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-error/10 flex items-center justify-center shrink-0">
              <HugeiconsIcon icon={RoboticIcon} className="w-4 h-4 text-error" />
            </div>
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-600 truncate mt-0.5">{entry.reasonLabel}</p>
            </div>
          </div>
        ))}
      </div>
